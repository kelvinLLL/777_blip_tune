...
def eval_model(model, processor, tokenizer, model_name, batch_size):
    ...
    question_filepath = "data/knowledges/metric3_questions.json"
    answer_filepath = "data/knowledges/metric3_annotations.json"
...
def eval_model3(model, processor, tokenizer, model_name, batch_size):
    knowledge_filepath =  "data/knowledges/metric3_knowledge.txt"
    question_filepath = "data/knowledges/metric3_questions.json"
    answer_filepath =  "data/knowledges/metric3_annotations.json"
    results = inference_model(
        model,
    ...
    result_file_path = EVAL_DIR + f"{model_name}_metric3_results.json"

    with open(result_file_path, "w") as result_file:
        json.dump(results, result_file)
    print(f"Results written to {result_file_path}")

    run_eval(answer_filepath, question_filepath, dataSubType="val2014", resultType=model_name)

def eval_model1(model, processor, tokenizer, model_name, batch_size):
    knowledge_filepath = "data/knowledges/metric1_knowledge.txt"
    question_filepath = "data/knowledges/metric1_questions.json"
    answer_filepath = "data/knowledges/metric3_annotations.json"
    results = inference_model(
        model,
        processor,
    ...
    result_file_path = EVAL_DIR + f"{model_name}_metric1_results.json"
...
def eval_model2(model, processor, tokenizer, model_name, batch_size):
    knowledge_filepath = "data/knowledges/metric2_knowledge.txt"
    question_filepath = "data/knowledges/metric2_questions.json"
    answer_filepath = "data/knowledges/metric3_annotations.json"
    results = inference_model(
        model,
        processor,
    ...
    result_file_path = EVAL_DIR + f"{model_name}_metric2_results.json"
...
if __name__ == "__main__":
    ...
    checkpoint_path = os.path.join(MODEL_DIR, f"okblip_freeze_bert6_vision_epoch3_lhj_origin.pt")
    model_dict = torch.load(checkpoint_path)
    
    model.load_state_dict(model_dict)

    eval_model2(model, processor, tokenizer ,f"metric3_lhj",4)
    eval_model1(model, processor, tokenizer ,f"metric1_lhj",4)
    eval_model3(model, processor, tokenizer ,f"metric2_lhj",4)
